...
with st.sidebar:
    ...
    st.title("Navigation")
    page = st.radio("Select Dashboard", ["Single Stock (LSTM + ARO)", "Multi-Stock Comparison"])
    st.markdown("---")

if page == "Single Stock (LSTM + ARO)":
    ...
    with st.sidebar:
        ...
        if data_source == "CSV Upload":
            ...
            start_date = st.date_input("Start Date", pd.to_datetime('2020-01-01'), key="single_start")
            end_date = st.date_input("End Date", pd.to_datetime('today'), key="single_end")
    ...
    with st.sidebar:
        ...
        start_date = st.date_input("Start Date", pd.to_datetime('2023-01-01'), key="multi_start")
        end_date = st.date_input("End Date", pd.to_datetime('today'), key="multi_end")
